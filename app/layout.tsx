import type { Metadata } from "next";
import "./globals.css";
import { siteConfig } from "../lib/site-config";
import { media } from "../lib/media";
import StickyMobileCTA from "../components/ui/StickyMobileCTA";
import AppShell from "../components/AppShell";

/**
 * Fonts are loaded via <link> in <head> rather than next/font/google.
 * This keeps the build free of a build-time fetch to Google's font API
 * (handy in network-restricted CI / sandboxed environments) while still
 * giving the browser the real family at runtime. Swap for next/font/google
 * at any time if you'd rather self-host / inline the font files.
 */

export const metadata: Metadata = {
  metadataBase: new URL(siteConfig.url),
  title: {
    default: `${siteConfig.name} | General Contractor in Central Texas`,
    template: `%s | ${siteConfig.name}`,
  },
  description:
    "Premium general contractor serving Greater Central Texas. Concrete, drywall, interior flooring, underground utilities, and residential & commercial construction. Insured and free estimates.",
  keywords: [
    "general contractor Central Texas",
    "general contractor Austin",
    "concrete contractor Central Texas",
    "concrete driveways Austin",
    "drywall installation Austin",
    "interior flooring Central Texas",
    "water and sewer line repair Austin",
    "residential construction Central Texas",
    "commercial construction Austin",
    "foundation repair Central Texas",
  ],
  authors: [{ name: siteConfig.name }],
  creator: siteConfig.name,
  openGraph: {
    type: "website",
    locale: "en_US",
    url: siteConfig.url,
    title: `${siteConfig.name} | General Contractor in Central Texas`,
    description:
      "Premium general contracting for Greater Central Texas. Concrete, drywall, flooring, underground utilities & more. Insured and free estimates.",
    siteName: siteConfig.name,
    images: [{ url: media.og.image, width: 1200, height: 630, alt: siteConfig.name }],
  },
  twitter: {
    card: "summary_large_image",
    title: `${siteConfig.name} | General Contractor in Central Texas`,
    description:
      "Premium general contracting for Greater Central Texas. Insured and free estimates.",
    images: [media.og.image],
  },
  robots: { index: true, follow: true },
  alternates: { canonical: siteConfig.url },
};

const jsonLd = {
  "@context": "https://schema.org",
  "@graph": [
    {
      "@type": ["GeneralContractor", "HomeAndConstructionBusiness"],
      "@id": `${siteConfig.url}/#business`,
      name: siteConfig.name,
      description: siteConfig.description,
      url: siteConfig.url,
      telephone: siteConfig.phoneHref.replace("tel:", ""),
      email: siteConfig.email,
      priceRange: "$$",
      areaServed: siteConfig.serviceAreaCities.map((city) => ({
        "@type": "City",
        name: city,
      })),
      address: {
        "@type": "PostalAddress",
        addressRegion: "TX",
        addressCountry: "US",
      },
      geo: {
        "@type": "GeoCoordinates",
        latitude: siteConfig.geo.lat,
        longitude: siteConfig.geo.lng,
      },
      openingHoursSpecification: {
        "@type": "OpeningHoursSpecification",
        dayOfWeek: ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"],
        opens: "07:00",
        closes: "18:00",
      },
      sameAs: [siteConfig.social.facebook, siteConfig.social.instagram],
    },
    {
      "@type": "WebSite",
      "@id": `${siteConfig.url}/#website`,
      url: siteConfig.url,
      name: siteConfig.name,
      publisher: { "@id": `${siteConfig.url}/#business` },
    },
  ],
};

export default function RootLayout({
  children,
}: Readonly<{ children: React.ReactNode }>) {
  return (
    <html lang="en">
      <head>
        <link rel="preconnect" href="https://fonts.googleapis.com" />
        <link rel="preconnect" href="https://fonts.gstatic.com" crossOrigin="anonymous" />
        <link
          rel="stylesheet"
          href="https://fonts.googleapis.com/css2?family=Big+Shoulders:wght@500;600;700;800;900&family=Inter:wght@400;500;600;700&family=IBM+Plex+Mono:wght@400;500&display=swap"
        />
        <script
          type="application/ld+json"
          dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }}
        />
      </head>
      <body className="antialiased bg-ink text-concrete">
        <AppShell>{children}</AppShell>
        <StickyMobileCTA />
      </body>
    </html>
  );
}
